...
int battle_calc_damage(struct block_list *src,struct block_list *bl,struct Damage *d,int damage,int skill_num,int skill_lv,int element)
{
	...
	if( sc && sc->count )
	{
		...
		if( sd && (sce = sc->data[SC_MILLENNIUMSHIELD]) && sce->val2 > 0 && damage > 0 )
		{
			...
			d->dmg_lv = ATK_BLOCK;
			sc_start(bl,SC_STUN,15,0,skill_get_time2(RK_MILLENNIUMSHIELD,sce->val1)); // There is a chance to be stuned when one shield is broken. Whats the official chance? [Rytech]
...
//	{ "mob_skill_use",                      &battle_config.mob_skill_use,                   1,      0,      1,              }, //Deprecated
	...
	{ "max_lv",                             &battle_config.max_lv,                          1000,     0,      150,            },
	{ "aura_lv",                            &battle_config.aura_lv,                         1000,     0,      INT_MAX,        },
